...
impl Parcel {
    ...
    /// ARO patch: write a bare file-descriptor object, the way Java's
    /// `Parcel.writeRawFileDescriptor` / `FileDescriptor` AIDL type does
    /// (no not-null marker, no comm-fd marker). The descriptor is dup'ed.
    pub fn write_raw_file_descriptor(&mut self, fd: std::os::fd::BorrowedFd<'_>) -> Result<()> {
